...
void KMeans::Cluster(double *data, int N, int *Label)
{
	...
	int unchanged = 0;
	bool loop = true;
	int* counts = new int[m_clusterNum];                // 统计每个聚类中样本的数目
	double** next_means = new double*[m_clusterNum];	// New model for reestimation
	for(int i = 0; i < m_clusterNum; i++)
	{
		next_means[i] = new double[m_dimNum];
	...
	while(loop)
	{
		...
		for(int i = 0; i < size; i++)
		{
			...
			currCost += GetLabel(x, &label);   // 统计代价 

			counts[label]++;                   // 统计每个聚类的样本数目
			for(int d = 0; d < m_dimNum; d++)
			{
				next_means[label][d] += x[d];  // 每个聚类里的样本求和
			}
		}
		...
		for(int i = 0; i < m_clusterNum; i++)
		{
			if(counts[i] > 0)
			{
				...
				memcpy(m_means[i], next_means[i], sizeof(double) * m_dimNum);  // 更新均值
			}
		}
